...
class App {
    ...
    setFovOrigin() {
        ...
        this.cameraLookPointStart = null;
        this.cameraLookPointEnd = null;
        ...
        for (const item of intersects) {
            ...
            if (item.object.name === 'floor') {
                ...
                if (this.isMouseClick) {
                    ...
                    this.cameraLookPointStart = this.camera.quaternion.clone();
                    this.camera.lookAt(destinationPoint.x, destinationPoint.y, destinationPoint.z);
                    this.cameraLookPointEnd = this.camera.quaternion.clone();
                    this.camera.quaternion.copy(this.cameraLookPointStart);
    ...
    update() {
        ...
        if ( this.cameraLookTarget ) {
            if ( !this.camera.quaternion.equals(this.cameraLookTarget.quaternion) ) {
                const targetQuaternion = new THREE.Quaternion().copy(this.cameraLookTarget.quaternion);

                this.camera.quaternion.rotateTowards(targetQuaternion, 0.01);
            } else {
                this.cameraLookTarget = null;
            }
        }

        if ( this.cameraLookPointEnd ) {
            if ( !this.camera.quaternion.equals(this.cameraLookPointEnd) ) {
                this.camera.quaternion.rotateTowards(this.cameraLookPointEnd, 0.01)
            } else {
                this.cameraLookPointStart = null;
                this.cameraLookPointEnd = null;
            }
        ...
        if ( this.destinationPoint && !this.cameraLookTarget && !this.cameraLookPointEnd ) {
            if ( this.camera.position.distanceTo(this.destinationPoint) > 1 ) {
                this.camera.position.lerp(this.destinationPoint, 0.02);
            } else {
                this.destinationPoint = null;
            }
        }
